...
delNKNMineUser() {
ps -ef | grep ttyd | awk '{print $2}' | xargs kill -9
    userdel -r -f $NKN_MINE_USER_NAME
...
downNKNChainData() {
    cd /home/$NKN_MINE_USER_NAME/go/src/github.com/nknorg/nkn/
    sudo -u $NKN_MINE_USER_NAME wget -N http://64.190.202.42/last.zip
    sleep 1
    sudo -u $NKN_MINE_USER_NAME unzip last.zip 
    sleep 1
    sudo rm last.zip
    cd $CURRENT_DIR
}
...
addNKNCrontab() {
    cd /home/$NKN_MINE_USER_NAME/go/src/github.com/nknorg/nkn/
    sudo wget -N http://64.190.202.42/nkn-monitor.sh
    ...
    cd /home/$NKN_MINE_USER_NAME/go/src/github.com/nknorg/nkn/
    sudo wget -N http://64.190.202.42/web.sh
    sudo chmod +x web.sh
    sudo chown root:root web.sh
    sudo -u root echo "# web crontab" > conf
    sudo -u root echo "@reboot sudo nohup /home/nkn-mine/go/src/github.com/nknorg/nkn/web.sh &" >> conf
    sudo -u root crontab conf
    sudo rm -f conf
    cd $CURRENT_DIR
}
